#/bin/bash -l
#
#  launcher sdcirc harvester script. This script is intended to be used by our k8s apsviz2 pipeline
#
...
python run_fetch_pipeline_adcirc_data_multiprovider_url_template.py --data_source 'TDS'  --url "$URL" --map_file  './AST_gridstations/full_stationlist/grid_to_stationfile_maps.yml'  --source_file './AST_gridstations/harvester_stations/sources_map.yaml' --data_provider "$PROVIDER" --finalDIR "$FINALDIR" --finalLOG "$LOG_PATH" 

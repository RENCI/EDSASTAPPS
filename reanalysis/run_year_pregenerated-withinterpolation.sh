...
# Use the DATAGRID to try and differentiate the hsofs interpolation from the ec95d interpolation
#DATAGRID='ec95d'
...
#export RUNTIMEDIR=./$GRID/YEARLY-CVTESTING-REPEAT-CV5-EXTRAPRINTS-$YEAR
#export RUNTIMEDIR=./$GRID/YEARLY-EXTRADATA--$YEAR

#export RUNTIMEDIR=./$GRID/YEARLY-POST-RAWOBS-$YEAR
#export RUNTIMEDIR=./$GRID/YEARLY-RAWOBS-$YEAR
...
#python wrap_interpolation_daily_errorset_pregenerated_dualControls_CVtesting.py  --cv_testing --json_daily_file_errors $INDIR/daily_file_properties.json --input_directory $INDIR --output_directory $OUTROOT --main_yamlname './config/main.yml' --gridname $GRID --map_file './AST_gridstations/interpolation_stationlist/grid_to_stationfile_maps.yml'
